...
RSpec.describe Brillo::SCRUBBERS do
  ...
  describe "name scrubber" do
    it "scrubs consistently across one run" do
      expect(Brillo::SCRUBBERS[:name].call(name)).to eq Brillo::SCRUBBERS[:name].call(name)
    end
  end

    describe "jumble scrubber" do
      it "scrubs consistently across one run" do
        expect(Brillo::SCRUBBERS[:jumble].call(name)).to eq Brillo::SCRUBBERS[:jumble].call(name)
      end
    end

  describe "benchmark ips" do
    it "performs well" do
      Benchmark.ips do |x|
        x.report "phone" do
